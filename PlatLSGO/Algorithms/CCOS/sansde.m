...
function [pop,val,bestmem,bestval,ccm] = sansde(Func, func_num, dim_index, pop, val, bestmem, bestval, Lbound, Ubound, FEmaxCycle, ccm)

    [popsize, dim] = size(pop);
    NP = popsize;
    D = dim;
    itermax = floor(FEmaxCycle/NP);
    tracerst = [];
    ...
    while iter < itermax
        ...
        for i=1:NP
            tempval(i) = Func(gpop(i,:)', func_num);
            if (tempval(i) <= val(i))
                if (tempval(i) < val(i))
