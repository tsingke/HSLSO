...
while(FEs < MaxFEs)
    ...
    winidxmask = repmat([1:popsize]', [1 dimension]); % m rows, d columns
    ...
    for i = 2:popsize
        if (rand < l(i))
            ...
            if mod(FEs, floor(MaxFEs/10)) == 0 && FEs <= MaxFEs
                fprintf("PCLSO  FE %d  best = %e\n",FEs,gbestfitness);
            end            
        end
    end

    if Stag >= 10000 
        [~,i] = min(fitness); % Targeting the global best
        for k=1:popsize
            ...
            index1 = (newp(i,:)>xmax)|(newp(i,:)<xmin); % Boundary control
