function [bestp,bestever,gbesthistory]=SLPSO(mainHandle,popsize,d,xmax,xmin,vmax,vmin,maxiter,Func,FuncId,VisualSwitch)
maxfe = 3e6;
lu = [xmin * ones(1, d); xmax * ones(1, d)];
M = 100;
m = M + floor(d/10);% 种群规模自适应，传入的popsize用不上
c3 = d/M*0.01;
PL = zeros(m,1);
for i = 1 : m
    PL(i) = (1-(i-1)/m)^log(sqrt(ceil(d/M))); % 学习概率，这里的i表示粒子适应度值的排名
end
% initialization
XRRmin = repmat(lu(1, :), m, 1);
XRRmax = repmat(lu(2, :), m, 1);
rand('seed', sum(100 * clock));
p = XRRmin + (XRRmax - XRRmin) .* rand(m, d); % 随机初始化
for i = 1:m
...
%% main loop
while(FES < maxfe)
    [fitness rank] = sort(fitness, 'descend'); % 降序排序（差到好）
    p = p(rank,:);
    v = v(rank,:);
    besty = fitness(m);
    bestp = p(m, :);
    ...
    randco2 = rand(m, d);
    randco3 = rand(m, d);
    winidxmask = repmat([1:m]', [1 d]); % m行，d列
